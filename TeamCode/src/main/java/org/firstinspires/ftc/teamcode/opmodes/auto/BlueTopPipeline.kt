...
/**
 * Blue Top Pipeline Autonomous -> TeleOp
 *
 * Starting position: Blue alliance, top position (near observation zone)
 * Autonomous: Simple preload score, then transitions to driver control
 */
@Suppress("UNUSED")
val aBlueTopPipeline = Mercurial.pipelineAutonomous {
    ...
    // Blue Top starting position (near observation zone)
    val startPoseBlueTop = Pose(40.0, 84.0, Math.toRadians(180.0))
    val blueScore = Pose(48.0, 48.0, Math.toRadians(180.0))

    lateinit var blueTopToScore: PathChain

    fun buildPaths() {
        blueTopToScore = container.follower.pathBuilder()
            .addPath(Path(BezierLine(startPoseBlueTop, blueScore)))
    ...
    fun autonomousPathUpdate() {
        when (pathState) {
            0 -> {
                ...
                container.follower.followPath(blueTopToScore)
                setPathState(1)
            }
            1 -> {
                if (!container.follower.isBusy) {
                    // Lock flywheel to distance and spin up
                    val distance = container.getGoalDistance(Alliance.BLUE)
                    if (distance != null) {
    ...
    sequence(
        ...
        loop({ pathState < 4 }, exec {
            ...
            // Telemetry
            telemetryA.addLine("=== BLUE TOP AUTONOMOUS ===")
